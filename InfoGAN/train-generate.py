...
from dataloader-generate import get_data
from utils import *
...
for epoch in range(params['num_epochs']):
    ...
    for i, (data, _) in enumerate(train_custom_data, 0):
        # Get batch size
        b_size = data.size(0)
        ...
        if i != 0 and i%100 == 0:
            print('[%d/%d][%d/%d]\tLoss_D: %.4f\tLoss_G: %.4f'
                  % (epoch+1, params['num_epochs'], i, len(train_custom_data), 
    ...
    if((epoch+1) == 1 or (epoch+1) == params['num_epochs']/2):
        ...
        plt.savefig("Epoch_N9_%d {}".format(params['dataset']) %(epoch+1))
        plt.close('all')

    # DON'T network weights
...
plt.axis("off")
plt.imshow(np.transpose(vutils.make_grid(gen_data, nrow=10, padding=2, normalize=True), (1,2,0)))
plt.savefig("Epoch_Nine_%d_{}".format(params['dataset']) %(params['num_epochs']))
